...
if not os.path.exists("pipeline.pkl"):
    st.error("No se encontró el archivo pipeline.pkl. Verifica que esté subido en el mismo nivel de app.py.")
    st.stop()

if os.path.getsize("pipeline.pkl") == 0:
    st.error("El archivo pipeline.pkl está vacío o corrupto. Súbelo nuevamente.")
    st.stop()
...
<div class="sidebar-copy">Modelo predictivo para estimar el riesgo de siniestros viales con víctimas.</div>
...
""", unsafe_allow_html=True)
    ...
    st.markdown("""
<div class="help-card">Ajusta las condiciones del escenario que prefieras y ejecuta la predicción para conocer el nivel de riesgo estimado.</div>
""", unsafe_allow_html=True)

st.markdown("""
<div class="top-label">SISTEMA INTELIGENTE DE RIESGO VIAL</div>
<div class="main-title">Predicción de riesgo de<br>siniestros viales en <span>Bogotá</span></div>
...
<div class="card-title">LECTURA DEL MODELO</div>
<div class="model-text">La salida representa la probabilidad estimada de riesgo alto según las variables disponibles en el modelo.</div>
<div class="model-list">• El modelo analiza patrones históricos de siniestros.<br>• Considera la hora, el mes y la localidad seleccionada.<br>• Útil para priorizar intervenciones y vigilancia vial.</div>
</div>
""", unsafe_allow_html=True)
...
if predecir:
    ...
    if prob_pct < 35:
        ...
        color = "#FFD166"
        texto = "El escenario presenta un nivel intermedio de riesgo. Se recomienda observar condiciones adicionales como congestión, clima o volumen vehicular."
    elif prob_pct < 80:
        titulo = "RIESGO ALTO"
        color = "#FF7B54"
        texto = "El escenario presenta una probabilidad elevada de riesgo alto. Se recomienda priorizar vigilancia preventiva y monitoreo continuo."
    else:
        titulo = "RIESGO CRÍTICO"
        color = "#FF4B5C"
        texto = "El escenario presenta una probabilidad crítica de riesgo alto. Se recomienda priorizar intervención y acciones preventivas inmediatas."
    ...
    if hora >= 18 or hora <= 5:
        insight.append("La franja horaria seleccionada corresponde a horas de menor visibilidad.")
    elif 6 <= hora <= 9:
    ...
    insight.append(f"La localidad analizada es {localidad}.")
...
""", unsafe_allow_html=True)
    ...
    st.markdown(f"""
<div class="cyber-card">
<div class="card-title">INTERPRETACIÓN DEL ESCENARIO</div>
<div class="model-text">{insight_texto}</div>
</div>
""", unsafe_allow_html=True)

    st.markdown(f"""
<div class="cyber-card">
<div class="card-title">CONFIANZA DEL MODELO</div>
<div class="insight-grid">
<div>
<div class="confidence-number">{confianza}%</div>
<div class="model-text">{nivel_confianza}</div>
</div>
<div class="model-text">{lectura_confianza}</div>
</div>
</div>
""", unsafe_allow_html=True)
